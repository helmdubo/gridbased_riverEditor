...
### 📌 Текущая сессия (обновлено 2025-11-12)

- [x] Исправить создание новой реки: вторая точка должна становиться устьем без повторного выделения истока
- [x] Обновить логику кнопки "New River", чтобы она создавалa отдельный сплайн без очистки сцены
- [x] Перейменовать сущности рек с `edge` на `spline` в пользовательском интерфейсе и внутренних структурах, где это отражает доменную модель
- [x] Исправить присвоение типов узлам после добавления притока (junction должен применяться только к точке соединения)
- [x] Переработать создание притока от произвольной вершины родителя (кроме source/junction) и логику отделения при удалении junction
