...
- [x] Превратить «New River» и отделённые притоки в полноценные родительские сплайны без отдельного состояния «симулянта».
- [x] Обеспечить корректное наследование ширины: главные реки в пикселях, притоки — процентами от родителя.
- [x] Настроить отрисовку ширины и UI так, чтобы независимые реки не масштабировались от основной.
- [x] Придать независимым сплайнам визуальный и поведенческий статус родительских рек (цвета, обработчики узлов).

🎯 **Итог:** все задачи текущей сессии закрыты; редактор поддерживает множество независимых рек, корректную работу притоков и консистентную ширину.

---

### 🔄 Следующие шаги (подготовлено для следующей сессии)

- [ ] **GraphAdapter 2.0:** перейти от временного адаптера `legacy`-структур к полноценной модели «граф из графов», чтобы отказаться от двойного формата данных.
- [ ] **FlowService перепись:** пересчитать потоки на основе `RiverGraphV2` без промежуточных преобразований, синхронизировать визуализацию скорости с шириной притоков.
- [ ] **Pointer capture & gestures:** довести до production-состояния обработку pointer capture в `RiverOverlay` и добавить поддержку stylus/pen устройств.
- [ ] **QA regression:** подготовить чек-лист smoke-тестов для мульти-речных сцен (3+ рек, каскад притоков, отделение/повторное присоединение).
- [ ] **Документация UE:** начать мэппинг функций core/graph в Blueprint Function Library (таблица соответствий).

> ℹ️ Перед началом новой сессии загляните в раздел «Agent Handoff» README для актуальных заметок и статуса тестов.
