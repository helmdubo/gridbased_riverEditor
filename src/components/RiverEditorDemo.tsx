...
export const RiverEditorDemo: React.FC<RiverEditorDemoProps> = ({ cols, rows }) => {
  ...
  const computeWidthPx = useCallback(
    (spline: Spline | null | undefined) => {
      ...
      if (spline.parentId) {
        ...
        if (parent && parent.attributes.width.kind === 'px' && parent.attributes.width.value > 0) {
          return (spline.attributes.width.value / 100) * parent.attributes.width.value;
        }
      }
  ...
  useEffect(() => {
    ...
    if (isTributary) {
      ...
      if (activeSpline.attributes.width.kind === 'relative') {
        percentValue = activeSpline.attributes.width.value;
      } else if (activeSpline.parentId) {
        const parent = GraphService.getSpline(riverGraph, activeSpline.parentId);
        if (parent && parent.attributes.width.kind === 'px' && parent.attributes.width.value > 0) {
          percentValue = (computeWidthPx(activeSpline) / parent.attributes.width.value) * 100;
        }
      }
  ...
  const legacyGraphForOverlay = React.useMemo(() => {
    ...
    for (const [splineId, spline] of Object.entries(riverGraph.splines)) {
      ...
      if (points.length > 0) {
        ...
        const widthSpec = spline.attributes.width;
              ...
              <div style={{ marginTop: '10px' }}>
                ...
                <div style={{ marginTop: '8px' }}>
                  ...
                  <div style={{ marginLeft: '10px', marginTop: '5px' }}>
                    ...
                    <div>Width: <span style={{ color: '#fbbf24' }}>{spline.attributes.width.kind === 'px' ? `${spline.attributes.width.value}px` : `${spline.attributes.width.value}%`}</span></div>
                    {spline.parentId && (
                      <>
